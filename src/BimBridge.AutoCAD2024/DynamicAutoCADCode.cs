...
" + code + @"
        ...
        using (var provider = new CSharpCodeProvider(new Dictionary<string, string> { ["CompilerVersion"] = "v4.0" }))
        {
            ...
            AddReference(parameters, typeof(Autodesk.AutoCAD.ApplicationServices.DocumentExtension).Assembly);
            ...
            var method = compiled.CompiledAssembly.GetType("BimBridge.Dynamic.UserScript", true)
