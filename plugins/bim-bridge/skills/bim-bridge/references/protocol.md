...
the legacy compatibility path `%APPDATA%\AEC Codex\instances\<instance-id>.json`.
...
- `POST /v1/execute`: enqueue a deadline-scoped read or write operation with an
  exact execution target when sent by the current Runtime.
- `GET /v1/requests/<request-id>`: inspect request state and result.
- `POST /v1/requests/<request-id>/cancel`: request cancellation.

Cancellation is confirmed only before an Autodesk operation enters the running
state. A client timeout requests cancellation but must not claim that already
running in-process code was forcibly stopped.
