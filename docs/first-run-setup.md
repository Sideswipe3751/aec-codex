...
1. Add the version-pinned AEC Codex repository marketplace and install the
...
4. Codex reports prerequisites, product versions, running applications, exact
   paths, release URL, and SHA-256.
...
| `needs_prerequisite` | Windows x64 or Python 3.11+ requirement is not met | Install the prerequisite manually |
| `needs_repair` | State is unreadable or an installed file is missing/changed | Review and approve repair |
...
| `healthy` | Version, prerequisites, and recorded file hashes pass | Continue to MCP discovery |
...
`~/.agents/plugins/marketplace.json`, or call `codex plugin add`.
...
step succeeds; a failure restores those backups and reports the original error.
...
`%LOCALAPPDATA%\AEC Codex\maintenance` and preserves the Marketplace plugin.
